...
export const submitData = async (req, res) => {
  ...
  try {
    ...
    device.isRaining = Boolean(isRaining);
    device.rainfallIntensity = Number(rainfallIntensity);
    device.floodLevel = Number(floodLevel);
    ...
    return res.status(200).json({
      success: true,
      message: 'Data submission saved successfully',
    });
  } catch (error) {
    await session.abortTransaction();
    console.error('submitData error:', error.message);
